...
export interface IntersectWorldOptions {
  mode?: number;
...
export default class Ray {
  public static readonly CLOSEST = 1;
  public static readonly ANY = 2;
   ...
   */
  public callback: (result: RaycastResult) => void;
   ...
   * @param  {RaycastResult} options.result
   * @param  {boolean} options.skipBackfaces
   * @param  {number} options.collisionFilterMask
   * @param  {number} options.collisionFilterGroup
   * @param  {Vec3} options.from
   * @param  {Vec3} options.to
   * @param  {Function} options.callback
   * @return {Boolean} True if the ray hit anything, otherwise false.
   */
  intersectWorld(world: World, options: IntersectWorldOptions = {}) {
  ...
  intersectBox(
    ...
    if (shape.convexPolyhedronRepresentation) {
...
var intersectHeightfield_index: number[] = [];
